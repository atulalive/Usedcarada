					...
					<div class="col-12">
						<div class="card">
							<div class="card-header">
                                ...
                                <button type="button" class="btn btn-success ml-auto" data-toggle="modal" data-target="#CarModal"><i class="fa fa-plus" aria-hidden="true"></i> Car Type</button>
							</div>
							<div class="card-body">
...
<script type="text/javascript">
  ...
  $(document).ready( function () {
    ...
    var CarTable = $('#tbl-students-data').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "<?php echo base_url('public/admin/CarType_loaddata');?>", // json datasource
            type: "post",
            data: {
        ...
        $(document).on('click','.CarBodiesSave',function(){
            ...
            if(name != '' || slug != ''){
                $.ajax({
                    url: '<?php echo base_url('public/admin/CarType_Save');?>',
                    type: 'post',
            ...
            if(this.name == "deleteButton") {
				Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.post(
                            '<?php echo base_url('public/admin/CarTypeDelete');?>',
                            {id: id}, 
                            function(result) {
                                CarTable.draw();
        ...
        $("#tbl-students-data tbody").on('click', '.edit', function() {
            ...
            if(this.name == "edit") {
                $.post(
                    '<?php echo base_url('public/admin/CarTypeEdit');?>',
                    {id: id}, 
                    function(result) {
